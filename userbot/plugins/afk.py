...
async def on_afk(event):
    ...
    if USER_AFK and not (await event.get_sender()).bot:  # pylint:disable=E0602
     ...
     #           afk_since = f"`{int(seconds)}s` **ago**"
        msg = None
        message_to_reply = f"__My Master Has Been In afk For__ `{total_afk_time}`\nWhere He Is: ONLY HE KNOWS. I also don't bother where he goes or not. " + \
            f"\n\n__I think He may be back in a few light years. Put 5000 Billion Dollars to meet with him in advance__\n**REASON**: {reason}" \
            if reason \
